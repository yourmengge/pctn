  ...
  ([
	...
	function (module, exports, __webpack_require__) {
	  ...
	  function getdata(option, callback, interactive) {
		...
		var url = location.protocol + '//pdfm.eastmoney.com/EM_UBG_PDTI_Fast/api/js';
	...
	function (module, exports, __webpack_require__) {
	  ...
	  var dealData = function (json, type, id) {
		...
		for (i = 0, len = changeTicks.length; i < len; i += 2) {
		  ...
		  if (i !== 0) {
			te.push({
			  "str": tickToStr(changeTicks[i - 1]) + "/" + tickToStr(changeTicks[i]),
			  "tick": preTicks
			});
			if (i === len - 2) {
			  te.push({
				"str": tickToStr(changeTicks[i + 1]),
				"tick": totalTicks
			  });
			}
		  } else {
			te.push({
			  "str": tickToStr(changeTicks[i]),
			  "tick": 0
			});
			if (i === len - 2) {
			  te.push({
		...
		if (jys != 2 && jys != 6 && jys != 13 && jys != 80) {
		  te = [];
		  te.push({
			"str": tickToStr(changeTicks[0]),
			"tick": 0
		  });
		  te.push({
			"str": tickToStr(changeTicks[changeTicks.length - 1]),
			"tick": totalTicks
		  });
		}
		//获取时间点对应的时间字符串数组 
	...
	function (module, exports, __webpack_require__) {
	  ...
	  var DrawXY = (function () {
		...
		function drawXMark(ctx, k_height, oc_time_arr) {
		  ...
		  var len = this.options.data.data.filter(function (item) {
	...
	function (module, exports) {
	  ...
	  function moveDot(n, num) {
		/*
		 * 当数据长度超过22位数，浏览器会自动使用科学计数法
		 * 需要对大数进行格式化处理
		 */
		var formatN = n;
		if (n.toString().indexOf("e+") > 0) {
		  var arr = n.toString().split("e+");
		  var frist = arr[0].split(".");
  
		  var zeroCount = parseInt(arr[1]); // 零个个数
		  var pointAtferNumCnt = frist[1] ? frist[1].length : 0; // 小数位的个数
  
		  var strConnet = frist[0] + (frist[1] ? frist[1] : "");
		  for (var i = 0, len = zeroCount - pointAtferNumCnt; i < len; i++) {
			strConnet += "0";
		  }
		  formatN = strConnet;
		}
	  ...
	  var ChartLine = (function () {
		...
		ChartLine.prototype.addInteractive = function () {
		  ...
		  function eventHanlder(winX, winY) {
			...
			if (cursor > dateArr.length - 1) {
			  cursor = dateArr.length - 1;
			}
	...
	function (module, exports, __webpack_require__) {
	  ...
	  function dealData(json, isCR, type, code) {
		...
		if (ticks.length === 7) {
		  ...
		  timeStrs.push({
			index: offset,
			value: toFormDateTime(AM_start)
		  });
		  if (AM_end === AM_start + totalTicks / 2) {
			timeStrs.push({
			  index: Math.floor(offset + totalTicks / 4),
			  value: toFormDateTime(AM_start + totalTicks / 4)
			});
		  }
		  timeStrs.push({
			index: Math.floor(offset + totalTicks / 2),
			value: toFormDateTime(AM_end) + "/" + toFormDateTime(PM_start)
		  });
		  if (AM_end === AM_start + totalTicks / 2) {
			timeStrs.push({
			  index: Math.floor(offset + totalTicks * 3 / 4),
			  value: toFormDateTime(PM_start + totalTicks * 1 / 4)
			});
		  }
		  timeStrs.push({
			index: offset + totalTicks,
			value: toFormDateTime(PM_end)
		  });
		  ...
		  timeStrs.push({
			index: 0,
			value: toFormDateTime(start)
		  });
		  timeStrs.push({
			index: Math.floor(middle - start),
			value: toFormDateTime(middle)
		  });
		  timeStrs.push({
			index: totalTicks,
			value: toFormDateTime(end)
		  });
		}
		...
		dateStrs.push({
		  index: 0,
		  value: toFormDate(json.data[0].split(",")[0].split(" ")[0])
		});
		//计算每个数据点
		for (var i = 0, item; item = json.data[i]; i++) {
		  ...
		  if (toFormDate(point.dateTime) != dateStrs[dateStrs.length - 1].value) {
			dateStrs.push({
			  index: i,
			  value: toFormDate(point.dateTime)
			});
		  }
		  result.data.push(point);
		}
		...
		if (type.toLowerCase() === "r") {
		  ...
		  if (code.charAt(code.length - 1) === "7") {
			...
			result.timeStrs.push({
			  index: dateStrs[1].index + timeStrs[Math.floor(timeStrs.length / 2)].index,
			  value: timeStrs[Math.floor(timeStrs.length / 2)].value
			});
		  }
		} else {
		  result.timeStrs = dateStrs.slice(1);
	...
	function (module, exports, __webpack_require__) {
	  ...
	  function getData(code, callback) {
		var url = location.protocol + "//nuyd.eastmoney.com/EM_UBG_PositionChangesInterface/api/js";
	...
	function (module, exports) {
	  ...
	  function intoArr(name, value, date, pricedigit) {
		...
		if (this[name] === undefined) {
		  this[name] = [{
			value: (value * 1.0).toFixed(pricedigit),
			date: date
		  }];
		} else {
		  this[name].push({
	  ...
	  var Interactive = (function () {
		...
		Interactive.prototype.markMA = function (canvas, type, datas, start, end, index, color_arr) {
		  ...
		  for (var item in datas) {
			dataObj.push({
			  value: datas[item].slice(start, end),
			  name: item
			});
		  }
		...
		Interactive.prototype.showTip = function (canvas, w_x, w_y, c_y, w_y_open, w_y_highest, w_y_lowest, obj) {
		  if (!this.options.tip) {
			...
			if (obj.percent > 0) {
			  mark = "+";
			  color = this.options.up_color;
			} else if (obj.percent == 0) {
			  mark = "";
			  color = "#666666";
			} else {
			  mark = "-";
			  color = this.options.down_color;
			}
	...
	function (module, exports, __webpack_require__) {
	  ...
	  var slideBar = function () {
		...
		if (arguments.length == 1) {
		  ...
		  var max = 0;
		  var min = 10000;
		  var len = data.data.filter(function (item) {
			return item !== undefined;
		  }).length;
		  for (var i = 0; i < len; i++) {
			arr.push({
			  date: data.data[i].date_time,
			  value: data.data[i].close
			});
			if (i == 0 || data.data[i].date_time.substring(0, 4) != data.data[i - 1].date_time.substring(0, 4)) {
			  arrYear.push({
				year: data.data[i].date_time.substring(0, 4),
				order: i
			  });
			}
	  ...
	  var ChartPie = (function () {
		...
		ChartPie.prototype.draw = function (callback) {
		  ...
		  var startOffset = this.options.startOffset,
			...
			point = {
			  x: that.options.point.x * dpr,
			  y: that.options.point.y * dpr
			},
		...
		ChartPie.prototype.addInteractive = function () {
		  ...
		  var dpr = this.options.dpr;
		  var point = {
			x: that.options.point.x,
			y: that.options.point.y
		  };
		...
		ChartLine.prototype.addInteractive = function () {
		  ...
		  function eventHanlder(winX, winY) {
			...
			if (that.options.series2) {
			  for (i = 0, len = series2.length; i < len; i++) {
				tipArr.push({
				  ...
				  data: ((series2[i].data[cursor] === undefined || series2[i].data[cursor] === null) ? "" : series2[i].data[cursor]) + (series2[i].suffix || ""),
				  name: series2[i].name,
				  y: series2[i].data[cursor] == false ? padding_top + c_1_height / 2 : padding_top + (c_1_height - c_1_height * (series2[i].data[cursor] - y_min2) / (y_max2 - y_min2)),
				  suffix: (series2[i].suffix || ""),
				  type: series2[i].type
				});
			  }
			...
			if (!that.options.interOption) {
			  ...
			  var top = (tipArr[0].y + tipArr[tipArr.length - 1].y) / 2 / dpr - 50;
